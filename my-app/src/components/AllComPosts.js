    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch user posts
                const postsResponse = await axios.get(`http://localhost:3001/Cp/${userid}`);
        ...
        <h4>All Activity</h4>
          {loading ? (
            ...
            {posts
              ...
              .map(post => (
                <li key={post._id}>
                  <CompanyPost post={post} userId={user._id} onDelete={handleDelete} />
                </li>
              ))}
          </ul>
          )}
